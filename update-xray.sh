...
# ---------------------------------------------------------
# HWID (Используем системный machine-id)
# ---------------------------------------------------------
if [ -f /etc/machine-id ]; then
    HWID="$(cat /etc/machine-id)"
else
    # Fallback, если machine-id отсутствует (редко)
    HWID="$(uuidgen | tr -d '-')"
fi
...
# Запуск парсера с проверкой кода выхода
...
BACKUP_DIR="/usr/local/etc/xrayВот полный исправленный код `update-xray.sh`.

**Основные изменения:**
1.  **HWID**: Используется только `/etc/machine-id`. Логика с отдельным файлом удалена для упрощения.
2.  **Безопасность парсера**: Добавлена проверка кода выхода `python3`. Если парсер падает, скрипт прерывается до попытки перезапуска сервиса.
3.  **Атомарность**: Конфиг генерируется во временный файл, проверяется, и только потом заменяет основной.
4.  **Исправление `grep`**: Добавлено `|| true` при парсинге `.dgst`, чтобы `set -e` не убивал скрипт, если формат файла изменится.
5.  **Бэкапы**: Добавлена очистка старых бэкапов (хранятся последние 5).
